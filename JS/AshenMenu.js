(function($) {
    var AshenMenu = {
        ...
        init: function(options) {
            ...
            $this.each(function(i) {
                var $this = $(this),
                settings = $.extend({},
                AshenMenu.defaults, o),
                $menu = $('.' + settings.menu);
                $this.on('mousedown',
                function(e) {
                    if (e.which !== 3 && $(e.target).parents('.AshenMenu').length < 1 && settings.click_to_close) {
                        $this.find('.AshenMenu').stop(true, false).animate({
                            opacity: 0
                        },
                        {
                            duration: 100,
                ...
                function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    AshenMenu.getCoords(e);
                    $('.AshenMenu_close_me').stop(true, false).animate({
                        opacity: 0
                    },
                    {
                        duration: 100,
    ...
    $.fn.AshenMenu = function(method, options) {
        if (AshenMenu[method]) {
            return AshenMenu[method].apply(this, Array.prototype.slice.call(arguments, 1))
        } else if (typeof method === 'object' || !method) {
            return AshenMenu.init.apply(this, arguments)
        } else {
